...
def load_env_files(base_dir: Path) -> None:
    """Load labeler_loop env in a repo-conventional way.

    Priority (later overrides earlier):
    - repo_root/.env/.env.labeler_loop (local, uncommitted)
    ...
    # base_dir -> apps/labeler_loop
    # repo_root -> AITuber/
    repo_root = base_dir.parents[1]
    env_dir = repo_root / ".env"

    load_env_file(env_dir / ".env", override=False)
    load_env_file(env_dir / ".env.labeler_loop", override=True)
