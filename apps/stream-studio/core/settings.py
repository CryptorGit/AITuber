...
def _find_repo_root(start: Path) -> Path:
    ...
    for parent in [p] + list(p.parents):
        try:
            ...
            continue
    # Fallback: assume 3 levels up from apps/stream-studio/core/settings.py
...
class Settings(BaseSettings):
    ...
    Non-secrets may additionally live in config/stream-studio/app.yaml (loaded by the server).
    """

    model_config = SettingsConfigDict(env_prefix="AITUBER_", extra="ignore")

    # Paths
    data_dir: Path = Field(default=Path("data/stream-studio"))
    obs_overlay_path: Path = Field(default=Path("data/stream-studio/obs/overlay.txt"))
    ...
    vlm_screenshot_path: Path = Field(default=Path("data/stream-studio/vlm/latest.png"))
