...
def _find_repo_root(start: Path) -> Path:
    ...
    for parent in [p] + list(p.parents):
        try:
            ...
            continue
    # Fallback: assume 3 levels up from apps/stream_studio/core/settings.py
...
class Settings(BaseSettings):
    ...
    Non-secrets may additionally live in config/stream_studio/app.yaml (loaded by the server).
    """
    ...
    vlm_screenshot_path: Path = Field(default=Path("data/stream_studio/vlm/latest.png"))
...
def _apply_console_settings(settings: Settings) -> None:
    try:
        from apps.stream_studio.core.storage import read_json
