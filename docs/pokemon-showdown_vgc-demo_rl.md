...
- `data/experiments/ppo_rolloutlen_ab/results.csv`（集計）
- `data/experiments/ppo_rolloutlen_ab/summary.json`（variantごとの簡易サマリ）
- `data/experiments/ppo_rolloutlen_ab/<run_id>/summary.json`（1ランごとの証拠）

各ランの生ログ/証拠:

- `logs/runs/<run_id>/e2e_train.log`（TS の start/progress/final を含む）
- `logs/runs/<run_id>/ppo_train_metrics.jsonl`（train が走った場合のみ行が増える）
- `logs/runs/<run_id>/dumps/`（fail-fast で dump が出た場合）
