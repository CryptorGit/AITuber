...
- （任意）VTube Studio を実際に動かす場合は `AITUBER_VTUBE_AUTH_TOKEN` と `config/main/live2d_hotkeys.yaml`
...
`data/events.jsonl` に `type=timing` が追記されます（同一 `request_id` で相関）。
...
生成物（gitignore対象）
- `data/events.jsonl`
- `data/manager/pending.json`
- `data/state.json`
- `data/obs/overlay.txt`
- `data/audio/*.wav`
...
4. `config/main/live2d_hotkeys.yaml` の各タグに hotkeyID を貼り付け
