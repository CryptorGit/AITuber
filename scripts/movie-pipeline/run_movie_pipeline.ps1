...
function Resolve-RepoRoot {
  ...
  while ($true) {
    $hasReadme = Test-Path (Join-Path $d 'README.md')
    $hasApps = Test-Path (Join-Path $d 'apps')
    $hasWeb = Test-Path (Join-Path $d 'web')
    if ($hasReadme -and ($hasApps -or $hasWeb)) { return $d }
    $parent = Split-Path -Parent $d
    if (-not $parent -or $parent -eq $d) { break }
    $d = $parent
  }
  throw "[run_movie_pipeline] Failed to locate repo root from: $StartDir"
}
...
$defaultAssetsRoot = Join-Path $repoRoot 'data/pokemon-showdown/vgc-demo/exports'
$defaultDataRoot = Join-Path $repoRoot 'data/movie-pipeline'

$geminiMaxTokensCmd = ''
if ($GeminiMaxOutputTokens -gt 0) {
  $geminiMaxTokensCmd = "if(-not `$env:MP_GEMINI_MAX_OUTPUT_TOKENS -or -not (`$env:MP_GEMINI_MAX_OUTPUT_TOKENS.Trim())){ `$env:MP_GEMINI_MAX_OUTPUT_TOKENS = '$GeminiMaxOutputTokens' };"
}

$serverCmd = @(
  "`$env:MP_PORT = $ServerPort;",
  "if(-not `$env:MP_ASSETS_ROOT -or -not (`$env:MP_ASSETS_ROOT.Trim())){ `$env:MP_ASSETS_ROOT = '$defaultAssetsRoot' };",
  "if(-not `$env:MP_DATA_ROOT -or -not (`$env:MP_DATA_ROOT.Trim())){ if(Test-Path -LiteralPath '$defaultDataRoot'){ `$env:MP_DATA_ROOT = '$defaultDataRoot' } };",
  $geminiMaxTokensCmd,
  "npm --prefix `"$serverDir`" run dev"
) -join ' '
