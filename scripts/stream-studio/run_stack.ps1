...
if (-not (Test-Path $py)) {
  Write-Host '[run_stack] .venv not found. Running scripts/stream-studio/run_dev.ps1 to create venv + install deps.' -ForegroundColor Cyan
...
$logsDir = 'data/stream-studio/logs'
...
Write-Host "Stop: .\scripts\stream-studio\stop_stack.ps1" -ForegroundColor Gray
