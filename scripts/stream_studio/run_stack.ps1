...
if (-not (Test-Path $py)) {
  Write-Host '[run_stack] .venv not found. Running scripts/stream_studio/run_dev.ps1 to create venv + install deps.' -ForegroundColor Cyan
  .\scripts\stream_studio\run_dev.ps1
...
$uvArgs = @('uvicorn','apps.stream_studio.server.main:app','--host',$HostAddr,'--port',[string]$Port)
